<div>
    ...
    <x-form-section submit="createApiToken">
        <x-slot name="title">
            {{ __('api.api_token_manager.create.title') }}
        </x-slot>

        <x-slot name="description">
            {{ __('api.api_token_manager.create.description') }}
        </x-slot>

        <x-slot name="form">
            <!-- Token Name -->
            <div class="col-span-6 sm:col-span-4">
                <x-label for="name" value="{{ __('api.api_token_manager.create.fields.token_name') }}" />
            ...
            @if (Laravel\Jetstream\Jetstream::hasPermissions())
                <div class="col-span-6">
                    <x-label for="permissions" value="{{ __('api.api_token_manager.create.fields.permissions') }}" />
        ...
        <x-slot name="actions">
            <x-action-message class="me-3" on="created">
                {{ __('api.api_token_manager.create.messages.created') }}
            </x-action-message>
    ...
    @if ($this->user->tokens->isNotEmpty())
        ...
        <div class="mt-10 sm:mt-0">
            <x-action-section>
                <x-slot name="title">
                    {{ __('api.api_token_manager.manage.title') }}
                </x-slot>

                <x-slot name="description">
                    {{ __('api.api_token_manager.manage.description') }}
                </x-slot>
                                ...
                                <div class="flex items-center ms-2">
                                    @if ($token->last_used_at)
                                        <div class="text-sm text-gray-400">
                                            {{ __('api.api_token_manager.manage.fields.last_used') }} {{ $token->last_used_at->diffForHumans() }}
                                        </div>
                                    @endif
    ...
    <x-dialog-modal wire:model.live="displayingToken">
        <x-slot name="title">
            {{ __('api.api_token_manager.modals.display_token.title') }}
        </x-slot>

        <x-slot name="content">
            <div>
                {{ __('api.api_token_manager.modals.display_token.description') }}
            </div>
    ...
    <x-confirmation-modal wire:model.live="confirmingApiTokenDeletion">
        <x-slot name="title">
            {{ __('api.api_token_manager.modals.delete.title') }}
        </x-slot>

        <x-slot name="content">
            {{ __('api.api_token_manager.modals.delete.description') }}
        </x-slot>
