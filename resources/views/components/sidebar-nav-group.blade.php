...
<li x-data="{ expanded: {{ ($active ?? false) ? 'true' : 'false' }} }">
    <button type="button"
        ...
        class="w-full text-start flex items-center gap-x-3.5 py-2 px-2.5 my-2 mr-2 text-sm text-slate-700 rounded-lg hover:bg-gray-100 dark:bg-gray-800 dark:hover:bg-slate-700 dark:text-slate-400 dark:hover:text-slate-300 dark:focus:outline-none dark:focus:ring-0 transition-all ease-in-out duration-500"
    >
        @if($icon)
